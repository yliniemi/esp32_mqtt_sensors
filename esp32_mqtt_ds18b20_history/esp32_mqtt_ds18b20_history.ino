...
void printLocalTime()
{
  ...
  csv += String("\n\r") + (timeinfo.tm_year + 1900) + "-" + zp(timeinfo.tm_mon + 1) + "-" + zp(timeinfo.tm_mday) + "T" + zp(timeinfo.tm_hour) + ":" + zp(timeinfo.tm_min) + ":" + zp(timeinfo.tm_sec);
  // csv += String("\n\r") + minutes + ";";
  sensors.requestTemperatures();
  for (int i = 0; sensorArray[i].name != 0; i++)
  {
    float temperature = sensors.getTempC(sensorArray[i].address);
    if (temperature != -127) csv += String(";") + temperature;
  }
  Serial.print(csv);
  Serial.println();
  SerialOTA.print(csv);
